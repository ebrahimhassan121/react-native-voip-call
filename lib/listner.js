...
const RNVoipCallTimeout='RNVoipCallTimeout';
//Ios
...
const answerCall = handler =>{
 const unsubscribe= eventEmitter.addListener(RNVoipCallPerformAnswerCallAction, (data) => {
    handler(data);
    // unsubscribe.remove();
  });
  return unsubscribe;
}
const removeAnswerCallListner=()=>eventEmitter.removeListener(RNVoipCallPerformAnswerCallAction);
const timeOutCall=handler=> {
  if(isIOS)return;
 return eventEmitter.addListener(RNVoipCallTimeout, (data) => {
    handler(data)}
  );
}
const endCall = handler =>
  eventEmitter.addListener(RNVoipCallPerformEndCallAction, (data) =>{
    ...
    handler( isIOS ? {callerId : uuids}:data)}
  );
